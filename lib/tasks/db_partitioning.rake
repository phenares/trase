namespace :db do
  namespace :partitioning do
    task create: :environment do
      ...
      context_ids.each do |context_id|
        create_partition_for_context('partitioned_flows', context_id)

        %w(inds quals quants).each do |attributes|
          create_partition_for_context("partitioned_flow_#{attributes}", context_id)
        end
      end
    end
  ...
  def ensure_partitioned_tables_exist
    %w(flows flow_inds flow_quals flow_quants).each do |table_name|
      ...
      unless ActiveRecord::Base.connection.table_exists? partitioned_table_name
        abort("#{partitioned_table_name} doesn't exist")
      end
    end
  end

  def create_partition_for_context(table_name, context_id)
    ...
    send(:"insert_#{table_name}", partition_name, context_id)
    send(:"index_#{table_name}", partition_name, context_id)
  ...
  def insert_partitioned_flows(partition_name, context_id)
    execute(
      <<~SQL
  ...
  def insert_partitioned_flow_inds(partition_name, context_id)
    execute(
      <<~SQL
        INSERT INTO #{partition_name}
  ...
  def insert_partitioned_flow_quals(partition_name, context_id)
    execute(
      <<~SQL
        INSERT INTO #{partition_name}
  ...
  def insert_partitioned_flow_quants(partition_name, context_id)
    execute(
      <<~SQL
        INSERT INTO #{partition_name}
  ...
  def index_partitioned_flows(partition_name, context_id)
  ...
  def index_partitioned_flow_inds(partition_name, context_id)
    create_index(partition_name, 'context_id')
    create_index(partition_name, 'flow_id')
    create_index(partition_name, 'ind_id')
  end

  def index_partitioned_flow_quals(partition_name, context_id)
    create_index(partition_name, 'context_id')
    create_index(partition_name, 'flow_id')
    create_index(partition_name, 'qual_id')
  end

  def index_partitioned_flow_quants(partition_name, context_id)
    create_index(partition_name, 'context_id')
    create_index(partition_name, 'flow_id')
